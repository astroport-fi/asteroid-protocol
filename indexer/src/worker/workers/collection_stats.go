...
func (w *CollectionStatsWorker) Work(ctx context.Context, job *river.Job[CollectionStatsArgs]) error {
	...
	INSERT INTO collection_stats(id, listed, floor_price, floor_price_1d_change, floor_price_1w_change, owners, supply, volume, volume_24h, volume_7d)
		SELECT 
			...
			(SELECT change FROM collection_floor_daily WHERE collection_id = @collectionId and NOW() - "date" <= interval '24 hours' LIMIT 1) as floor_price_1d_change,
			(SELECT change FROM collection_floor_weekly WHERE collection_id = @collectionId and NOW() - "date" <= interval '7 days' LIMIT 1) as floor_price_1w_change,
