...
export class BuyWizardModalPage implements OnInit {
  ...
  wizardStep: "deposit" | "buy" | "inflight-deposit" | "inflight-buy" | "failed" | "invalid" | "success" = "deposit";
  errorText: string = "";
  listing: any = null;
  status: any = null;
  ...
  async ngOnInit() {
    ...
    let overrideFee = (environment.fees.protocol.marketplace["buy"] as any).amount;
    if ((environment.fees.protocol.marketplace["buy"] as any).type == 'dynamic-percent') {
      const feePercentage = parseFloat((environment.fees.protocol.marketplace["buy"] as any).amount);
  ...
  async buy(simulateOnly: boolean = true) {
    ...
    const urn = this.protocolService.buildURN(environment.chain.chainId, 'buy.' + this.marketplaceType, params);
