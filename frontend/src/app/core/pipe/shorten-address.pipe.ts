...
export class ShortenAddressPipe implements PipeTransform {
    ...
    transform(value: string, startLength: number = 10, endLength: number = 6): Observable<string> {
        ...
        return new Observable((observer) => {
            ...
            api<StargazeResponse>(`${environment.api.stargazeNameEndpoint}${queryBase64}`).then((response) => {
                if (response.data) {
